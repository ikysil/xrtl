...
type
  ...
  TbzlibBlockSize100k = (bs1, bs2, bs3, bs4, bs5, bs6, bs7, bs8, bs9);

function bzCompressInit(var strm: TBZStreamRec; blockSize100k: Integer;
  verbosity: Integer; workFactor: Integer): Integer; stdcall;

function bzCompress(var strm: TBZStreamRec; action: Integer): Integer; stdcall;

function bzCompressEnd(var strm: TBZStreamRec): Integer; stdcall;
...
function bzDecompress(var strm: TBZStreamRec): Integer; stdcall;

function bzDecompressEnd(var strm: TBZStreamRec): Integer; stdcall;
...
type
  EbzlibError = class(Exception);

function  bzlibCheck(Code: Integer): Integer;
...
const
  _bz_errmsg: array[0 .. 13] of PChar = (
    'Stream end',             // BZ_STREAM_END = 4;
    'Finish ok',              // BZ_FINISH_OK = 3;
    'Flush ok',               // BZ_FLUSH_OK = 2;
    'Run ok',                 // BZ_RUN_OK = 1;
    'Ok',                     // BZ_OK = 0;
    'Sequence error',         // BZ_SEQUENCE_ERROR = (-1);
    'Parameter error',        // BZ_PARAM_ERROR = (-2);
    'Memory error',           // BZ_MEM_ERROR = (-3);
    'Data error',             // BZ_DATA_ERROR = (-4);
    'Magic header is absent', // BZ_DATA_ERROR_MAGIC = (-5);
    'Input/output error',     // BZ_IO_ERROR = (-6);
    'Unxpected end of file',  // BZ_UNEXPECTED_EOF = (-7);
    'Output buffer is full',  // BZ_OUTBUFF_FULL = (-8);
    ''
  );
...
begin
  raise EbzlibError.CreateFmt('bzlib error %d', [errcode]);
end;
...
begin
  Result:= Code;
  if code < 0 then
    raise EbzlibError.Create(_bz_errmsg[4 - Code]);
end;
...
begin
  ...
  try
    ...
    bzlibCheck(bzCompressInit(strm, 9, 0, 0));
    try
      while bzlibCheck(bzCompress(strm, BZ_FINISH)) <> BZ_STREAM_END do
      begin
        P := OutBuf;
        Inc(OutBytes, 256);
...
begin
  ...
  try
    ...
    strm.next_out := OutBuf;
    strm.avail_out := OutBytes;
    bzlibCheck(bzDecompressInit(strm, 0, 0));
    try
      while bzlibCheck(bzDecompress(strm)) <> BZ_STREAM_END do
      begin
        P := OutBuf;
        Inc(OutBytes, BufInc);
        ReallocMem(OutBuf, OutBytes);
      ...
      bzlibCheck(bzDecompressEnd(strm));
    end;
    ReallocMem(OutBuf, strm.total_out);
    OutBytes := strm.total_out;
  except
